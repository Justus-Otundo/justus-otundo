    ...
    <section id="about" className="py-16 sm:py-20 lg:py-24 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start">
          ...
          >
            {/* Bio Card */}
            <div className="bg-gradient-to-br from-dark-800/80 to-dark-900/80 rounded-3xl p-5 sm:p-7 lg:p-8 border border-stone-700/50">
              <div className="flex items-start gap-4 sm:gap-5 mb-6">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-accent-400 font-semibold text-xs sm:text-sm">{profile.title}</p>
                  <p className="text-stone-500 text-xs sm:text-sm mt-1">Previously @ {profile.company}</p>
                  <p className="text-stone-500 text-xs sm:text-sm mt-1">{contact.locationFull}</p>
                </div>
              </div>
              ...
              <p className="text-stone-400 text-sm sm:text-base leading-relaxed mb-4">
                Moving from developer to technology lead means I understand both sides: the code and the business.
              ...
              <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-dark-700 hover:bg-dark-600 border border-stone-600 text-white font-semibold rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-accent-500"
                >
                  <ArrowDownTrayIcon className="w-5 h-5" />
                  Cover Letter
                </a>
          ...
          >
            {/* Contact Info Card */}
            <div className="bg-gradient-to-br from-dark-800/80 to-dark-900/80 rounded-3xl p-6 border border-stone-700/50">
              ...
              <div className="space-y-3">
                <a href={`mailto:${contact.email}`} className="flex items-center gap-3 p-3 bg-dark-700/50 rounded-xl hover:bg-dark-600/50 transition-colors">
                  ...
                  <div>
                    <p className="text-stone-200 text-sm font-medium">{contact.email}</p>
                  </div>
                </a>
                <a href={`tel:${contact.phoneLink}`} className="flex items-center gap-3 p-3 bg-dark-700/50 rounded-xl hover:bg-dark-600/50 transition-colors">
                  ...
                  <p className="text-stone-200 text-sm font-medium">{contact.phone}</p>
                </a>
              ...
              <div className="mt-4 pt-4 border-t border-stone-700/50 flex gap-3">
                ...
                <a href={social.github} target="_blank" rel="noopener noreferrer" aria-label="GitHub" className="w-11 h-11 flex items-center justify-center rounded-lg bg-dark-700 border border-stone-600 text-stone-400 hover:text-white hover:border-accent-500/50 transition-colors focus:outline-none focus:ring-2 focus:ring-accent-500">
            ...
            <div className="bg-gradient-to-br from-dark-800/80 to-dark-900/80 rounded-3xl p-6 border border-stone-700/50">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-xs text-stone-500 ml-auto">{certifications.length} earned</span>
              </div>
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1 scrollbar-thin">
                {certifications.map((cert) => (
                  ...
                  >
                    ...
                    <div>
                      <span className="text-stone-300 text-xs font-medium block leading-tight">{cert.name}</span>
                      <span className="text-stone-500 text-xs">{cert.issuer}{cert.year ? ` · ${cert.year}` : ''}</span>
                    </div>
                  </div>
                ))}
              </div>
